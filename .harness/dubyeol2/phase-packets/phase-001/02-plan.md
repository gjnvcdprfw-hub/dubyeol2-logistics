...
| 날짜 | 이탈 | 사유 |
|---|---|---|
| 2026-07-04 | vitest.config.ts → .mts | 플랜 코드가 CJS 로드로 실행 불가 (스펙 검토 발견) |
| 2026-07-04 | 스캐폴드에 `@/*` alias 유지 (플랜 플래그는 no-import-alias) | 플랜 Task 4·6 코드 자체가 `@/lib` import 사용 — 플랜 플래그 오기 |
| 2026-07-04 | 인증 오류 처리 강화: ValidationError 구분·register 500 일반화·prod SESSION_SECRET 강제 | 플랜 명세 코드의 결함 (품질 검토 I-1·I-2) |
| 2026-07-04 | orders.ts도 ValidationError 사용 + /api/orders POST 동일 오류 패턴 | 인증 태스크 패턴과 일관성 (팀리더 사전 승인) |
| 2026-07-04 | vitest `fileParallelism: false` | auth·orders 테스트가 같은 SQLite를 병렬 deleteMany하며 간헐 실패 — 최소 수정 (팀리더 사후 승인) |
| 2026-07-04 | 격리 이중 방어: lib sellerId 가드 + 페이지 자체 redirect (`session.userId!` 단언 제거) | 플랜 결함 — Prisma가 undefined 필터 무시해 전 셀러 노출 경로 실증 (스펙 검토 발견) |

리스크 이월(Phase 3 착수 시 처리): POST /api/orders가 body를 스프레드로 create에 전달 — status 등 임의 필드 주입 가능(mass assignment). Phase 1은 status가 무의미해 실해 없음, 견적/상태 전이가 생기는 Phase에서 화이트리스트 필수.
