...
|---|---|---|---|---|---|
| 1 | 가입·로그인 → 주문 접수(유료 검수 선택) → 내 주문 목록 확인 | 주문 접수 · 대시보드(주문 목록) | "주문을 맡길 수 있다"가 모든 것의 입구 | **경계 확정, plan 작성 중** | `phase-packets/phase-001/02-plan.md` |
| 2 | 운영자 입고·확인 기록 → 셀러가 입고 증거(사진·외포장·유료 검수 결과) 확인 | 대시보드(입고 관리) | "내 물건을 증거로 확인한다" | 준비 전 | `phase-packets/phase-002/00-customer-outcome.md` |
| 3 | 항목별 견적(두리무역 요율 초기값) 확인 → Feature 1 합격 예시 전체 통과 | 대시보드(견적) | "총비용을 알고 보낼지 결정한다" | 준비 전 | `phase-packets/phase-003/00-customer-outcome.md` |
