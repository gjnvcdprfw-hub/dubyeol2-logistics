...
- 현재 버전: v2.23
- 승격일: 2026-07-05
- v2.23 적용 범위: 실행 엔진 모드 스위치 — `engine.md` 신설(역할 배정 단일 정본). 모드 A(기본)=빌드 Claude/감리 Codex, 모드 B=빌드 Codex/감리 Claude 복귀 후 큐 처리(자가감리 금지·교차 감리, 대표님 A안). 전환=대표님 선언 한마디(개정 절차 불요), 감리 대기 큐 external-audit/pending/, 감리 PASS 전 머지 금지 유지. CLAUDE/AGENTS 포인터+필수 정본 등재. 설치본 3곳 현재 모드 B(2026-07-05 대표님 "코덱스위주로"). 계기: Claude 사용량 소진, "언제든 스위치". (보강 2026-07-05: 모드 B 감리 배선 명문화 — claude -p sonnet 무인 교차 감리, 물류 Phase 6 실감리 PASS로 실측 확정)
...
| 실행 엔진 모드 스위치 v2.23 (engine.md) | 정본 (중앙+twostarhub+white7+물류) | `revisions/engine-mode-v2.23-20260705.md` |
| 엔진 모드 v2.23 드라이런 (모드 B/복귀/정합) | PASS | `dry-runs/engine-mode-v2.23-dry-run-20260705.md` |

## 동기화 대상

| 프로젝트 | 적용 버전 | 상태 | 검증 |
|---|---|---|---|
| 중앙 하네스 | v2.23 | 적용 | `AGENTS.md`, `CLAUDE.md`, `VERSION.md` 확인 |
| twostarhub | v2.23 (모드 B) | 동기화 완료 (2026-07-04) | v2.21 감리 신뢰성 + v2.22 상한 폐지 동기화, 라이브 파일 보존 |
| white7 | v2.23 (모드 B) | 동기화 완료 (2026-07-04) | v2.21+v2.22 동기화, 상태판 규칙 사본 1줄 정합 |
| 물류 | v2.23 (모드 B) | 동기화 완료 (2026-07-04) | v2.21+v2.22 동기화. 디자인 정본 alpha(두리무역 실측), git 미초기화 |
