...
- 동시 운영 서브에이전트 수 상한 없음 — 동시 수는 태스크 의존성과 충돌 위험을 보고 Team Leader가 판단(구 최대 3개 상한은 Codex 빌더 시절 규칙, v2.22 폐지)
- 완료된 서브에이전트는 열어두지 않고 결과를 수집한 즉시 닫는다.
...
2. 프리플라이트 헬스체크: 감리 직전 아래 한 줄이 60초 내 "OK"를 돌려주는지 확인한다(실측 수 초). 실패하면 감리를 시작하지 않고 "감리 불가/재시도"로 표면화한다 — FAIL/BLOCKED가 아니다. 조치는 Codex 데스크톱 앱 완전 종료(app-server 클린) 후 재시도, 반복되면 PM 경유 대표님 보고.
   ```
   codex exec --sandbox read-only --skip-git-repo-check --model gpt-5.5 \
     --ephemeral --ignore-user-config "reply OK"
   ```
3. 빌드 측이 아래 명령으로 Codex 외부감리를 무인 호출한다.
   ```
   codex exec --sandbox read-only --skip-git-repo-check --model gpt-5.5 \
   ...
   ```
4. Codex는 `request.json`을 읽고 `result.schema.json`에 맞춰 `external-audit/result.json`(verdict·summary_for_owner·findings·question_for_pm_to_ask_owner)을 쓴다. read-only 샌드박스라 코드는 보기만 하고 못 고친다.
5. 빌드 측(Claude)이 `result.json`을 읽어 `03-verification.md`에 연결하고 PM에게 넘긴다.

격리 플래그의 뜻(2026-07-04 실측, v2.21): `--ignore-user-config`는 사람용 Codex 앱 설정(훅·플러그인·스킬·MCP)이 무인 감리에 끼어드는 것을 원천 차단한다 — 프로젝트 `AGENTS.md`와 구독 인증은 유지된다. `--ephemeral`은 세션 파일을 남기지 않아 공유 상태 간섭을 줄인다. `-c 'approval_policy="never"'`는 무인 실행을 보장한다(사용자 config이 `on-request`로 덮는 것을 방지 — "exec는 never가 기본"이라는 과거 단정은 실측 반증됨). `-c 'model_reasoning_effort="high"'`는 ignore-user-config로 낮아지는 추론 강도를 복구한다. 모델은 재현성을 위해 `--model gpt-5.5`로 고정한다.

호출 응답이 길어도 중단하지 않는다. `codex exec`가 오래 걸리면 백그라운드로 돌려서라도 완료(result.json 생성)를 기다린다. 시간 지연만으로 호출을 죽이거나 FAIL/BLOCKED로 판단하지 않는다. 단 hang 신호 3종이 모두 확인되면 죽이고 "감리 불가/재시도"로 처리한다(판별 세칙은 `## 시간 판단 금지`).
...
hang vs 느림 판별(외부감리 한정, v2.21): ① `-o` 출력 파일(result.json) mtime 미변 ② 프로세스 0% CPU 10분 이상 지속 ③ 출력에 `SessionStart Failed` 등 훅 실패 — 세 신호가 **모두** 확인되면 hang이다. 이때는 프로세스를 종료해도 되지만 결과는 FAIL/BLOCKED가 아니라 "감리 불가/재시도"로 기록하고, 프리플라이트부터 다시 시작한다. 재발하면 Codex 데스크톱 앱을 완전 종료(app-server 클린)한 뒤 재시도한다. exec 출력 로그가 계속 자라는 중이면 정상(느림)이니 기다린다. "세션 로그 없음"만으로 hang 판단은 금지한다 — 세션 로그 감지는 늦고 `--ephemeral`에서는 애초에 생성되지 않는다. 판정은 exec 출력 파일과 result.json 갱신으로만 한다. 사람용 Codex 화면에서 `hook: SessionStart Failed`가 보이면 해당 session_start 훅을 `enabled = false`로 끄고 기록한다(훅 실패는 codex 시작을 조용히 막는다).
