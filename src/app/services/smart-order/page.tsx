...
const FLOW: [string, string][] = [
  ["엑셀 업로드", "스마트스토어·쿠팡 주문 엑셀을 그대로 올립니다."],
  ["AI 파싱·매칭", "주문 행을 자동 구조화하고 1688 상품·옵션 매칭을 추천합니다."],
  ["확인·주문", "매칭 결과를 확인하면 구매가 진행됩니다."],
  ["입출고·운송장 회신", "입고·출고를 추적하고 국내 운송장 회신 엑셀을 받습니다."],
];
...
const FEATURES: [string, string][] = [
  ["엑셀 자동 파싱", "플랫폼 주문 엑셀을 업로드하면 주문 행이 자동으로 구조화됩니다."],
  ["1688 옵션 매칭 추천", "상품·옵션 매칭 후보를 추천하고, 고객 확인 후에만 주문합니다."],
  ["매칭 라이브러리", "한 번 확정한 매칭은 저장되어 다음 주문에 자동 재사용됩니다."],
  ["운송장 회신 엑셀", "출고 후 국내 운송장 번호를 플랫폼 업로드용 엑셀로 돌려드립니다."],
];

const STEPS: [string, string][] = [
  ["주문 엑셀 업로드", "플랫폼에서 내려받은 주문 엑셀을 올립니다."],
  ["자동 파싱 확인", "구조화된 주문 목록에서 확인 필요 건만 검토합니다."],
  ["매칭 확정", "추천된 1688 상품·옵션 매칭을 확정합니다."],
  ["구매 진행", "확정된 주문이 일괄 구매로 진행됩니다."],
  ["입고·출고 추적", "창고 입고와 한국행 출고를 대시보드에서 추적합니다."],
  ["운송장 회신", "국내 운송장 회신 엑셀을 받아 플랫폼에 업로드합니다."],
];
    ...
    <main className="bg-bg text-body">
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        ...
        <p className="mt-5 text-lg text-secondary max-w-2xl">
          플랫폼 주문 엑셀을 올리면 1688 매칭부터 구매, 입출고 추적, 국내 운송장 회신까지
          하나의 흐름으로 처리합니다.
        </p>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        ...
        <p className="mt-4 text-xs text-muted">
          구매 방식은 구매대행(수수료 상품가의 {RATES.commissionRate * 100}% + VAT) 기준이며, 요율은 서비스 준비 중 기준으로 변경될 수 있습니다.
        </p>
      </section>
