...
import { rejectWalletTopUpRequest } from "@/lib/wallet-topups";
...
export async function POST(req: Request, context: RouteContext) {
  ...
  try {
    const request = await rejectWalletTopUpRequest(user.id, id, await readReason(req));
    if (wantsJson(req)) {
      return NextResponse.json({ ok: true, request });
    }
    return NextResponse.redirect(new URL("/admin/wallet-topups?done=1", req.url), 303);
  } catch (error) {
    const message = error instanceof ValidationError ? error.message : "충전 요청 거절 중 오류가 발생했습니다";
    if (wantsJson(req)) {
